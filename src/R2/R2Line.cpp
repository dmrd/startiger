...
R2Line::R2Line(const R2Line& line) :
    vector(line.vector),
    normal(line.normal),
    c(line.c)
{
...
R2Line::R2Line(double a, double b, double c) :
    vector(-b, a),
    normal(a, b),
    c(c)
{
...
R2Line::R2Line(const R2Point& point, const R2Vector& vector, bool normalized)
{
...
R2Line::R2Line(const R2Point& point1, const R2Point& point2)
{
...
R2Line::R2Line(double x1, double y1, double x2, double y2)
{
...
void R2Line::Mirror(const R2Line& line)
{
...
void R2Line::Translate(const R2Vector& vector)
{
    // Move line by vector
...
void R2Line::Rotate(const R2Point& origin, double angle)
{
...
void R2Line::Reset(const R2Point& point, const R2Vector& vector, bool normalized)
{
    // Build line from point and vector
    // Normalize vector if not told that it already is normalized
    this->vector = vector;
...
R2Line& R2Line::operator=(const R2Line& line)
{
    // Copy everything
    vector = line.vector;
    normal = line.normal;
...
void R2Line::Print(FILE *fp) const
{
